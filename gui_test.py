...
def OpenEncodeImageInImageFrame():
    ...
    label_inputCoverPath = tk.Label(frm_encode,text='Cover file path...',relief=tk.GROOVE,width=50,anchor='w')
...
def openPayloadEncodedImage():
    ...
    #         label_image = Label(frm_encode_options, image = img)
    #         label_image.image=img
    ...
    #         videoplayer = TkinterVideo(master=frm_encode_options, scaled=True)
    #         videoplayer.load(filePath)
    #         videoplayer.grid(row=8,column=0,padx= 0,pady= 0)
    #         videoplayer.play() # play the video
    #     elif filePath.lower().endswith('.txt'):
    #         pathh = Entry(frm_encode_options)    
    #         pathh.insert(END, filePath)
    #         filePath = open(filePath) 
    #         data = filePath.read()
    #         txtarea = Text(frm_encode_options, width=40, height=10)
